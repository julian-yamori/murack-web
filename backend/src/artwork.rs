...
pub async fn get_original_artwork(
    Path(id): Path<i32>,
    State(pool): State<PgPool>,
) -> ApiResult<Response> {
    let record = sqlx::query!("SELECT image, mime_type from artworks WHERE id = $1", id)
        .fetch_optional(&pool)
        .await?;
